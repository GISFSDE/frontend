...
export default {
  ...
  mounted() {
    ...
    // 一、各类图层加载
    // 1.天地图 http://lbs.tianditu.gov.cn/server/MapService.html
    const imageURL = "http://t0.tianditu.gov.cn/img_w/wmts?" +
      "SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=img&STYLE=default&TILEMATRIXSET=w&FORMAT=tiles" +
      "&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}" +
      "&tk=e6372a5333c4bac9b9ef6097453c3cd6";
    const vectorURL = "http://t0.tianditu.gov.cn/vec_w/wmts?" +
      "SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=vec&STYLE=default&TILEMATRIXSET=w&FORMAT=tiles" +
      "&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}" +
      "&tk=e6372a5333c4bac9b9ef6097453c3cd6";
    var basemap1 = L.tileLayer(imageURL, {
      attribution: "stamen"
    });
    var basemap = L.tileLayer(vectorURL, {
      attribution: "stamen"
    });
    ...
    self.map.on("click", function (e) {
      usaDynamicLayer
        ...
        .run(function (error, featureCollection) {
          ...
          //   iconSize: [38, 95], // icon的大小
          //   shadowSize: [50, 64], // 阴影的大小
          //   iconAnchor: [22, 94], // icon的渲染的位置（相对与marker）
          //   shadowAnchor: [4, 62],  // shadow的渲染的位置（相对于marker）
          //   popupAnchor: [-3, -76] //若是绑定了popup的popup的打开位置（相对于icon）
          // });
          // this.locationMarker = L.marker([this.myPosition.latitude, this.myPosition.longitude], { icon: greenIcon }).addTo(this.map);
